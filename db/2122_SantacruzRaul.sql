...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 16-12-2021 a las 15:35:49
...
--
  CREATE TABLE `bd_restaurante_rsc`;
...
(25, '2021-12-31', '18:00:00', 'Hola Buenas Tardes jajajajisjaisji', 10);
...
--
DELIMITER $$
CREATE TRIGGER `ActuInci` AFTER UPDATE ON `tbl_incidencia` FOR EACH ROW BEGIN 
    UPDATE `tbl_copia` SET `data_incidencia` = NEW.data_incidencia, `hora_incidencia` = NEW.hora_incidencia, `desc_incidencia` = NEW.desc_incidencia, `id_mesa` = NEW.id_mesa WHERE `id_incidencia` = NEW.id_incidencia;
  END
$$
DELIMITER ;
DELIMITER $$
